
# Built in templates

TSPlate's main value comes from the templates and template helper functions build in to the library, which speed up the definition of your own templates.

## Index

1. [Basic types](./templates/basic-types.md)
2. [Arrays](./templates/arrays.md)
3. [Enums](./templates/enums.md)
5. [Objects](./templates/objects.md)
4. [Classes](./templates/classes.md)
5. [Helper Functions](./templates/helper.md)

### [Next - Basic Types →](./templates/basic-types.md)
### [← Previous - Creating a basic template](./creating-a-basic-template.md)
### [🏠 Home](./introduction.md)

